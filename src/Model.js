...
function CameraRig({ activeView, setActiveView }) {
  ...
  // Store previous camera position for smooth transitions
  const prevPosition = useRef(cameraViews.default.position);
  const cameraTarget = useRef(new THREE.Vector3());
  
  useFrame((state, delta) => {
    const targetView = cameraViews[activeView] || cameraViews.default;
    
    // Update previous position for smoother transitions
    prevPosition.current = state.camera.position.toArray();
    
    // Slower easing for smoother animation
    easing.damp3(
    ...
    // Smooth lookAt transition using the ref
    easing.damp3(
